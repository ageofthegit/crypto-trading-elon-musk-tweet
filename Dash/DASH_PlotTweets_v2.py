...
days_behind = st.sidebar.slider('Historical Days to plot', 1 , 25, (2,5))
...
interim_twitter_df = funcs.get_twitter_data(user = username, num_tweets = 200, stringsearch = 'doge')
runtimestr = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")
interim_twitter_df.to_csv('D:\OneDrive\Trading\Tweets\{}_tweets_{}.csv'.format(username, runtimestr), index = False)


twitter_df = funcs.import_all_csvfiles_into_df(path = 'D:\OneDrive\Trading\Tweets', drop_dups = True)


st.write('Twitter df shape after importing all tweets from folder is ', twitter_df.shape)
...
if st.sidebar.checkbox("Doge Tweets"):
     twitter_df = twitter_df[twitter_df.has_strsearch]
if st.sidebar.checkbox("Remove replies"):
     twitter_df = twitter_df[ twitter_df.reply_to.isnull() ]

print('twiter_df has shape after removing misc tweets of '.format(twitter_df.shape))
...
now_tm_utc = datetime.datetime.utcnow()

start_tm_utc = ( now_tm_utc + datetime.timedelta(minutes= -(days_behind[1] -1)*24*60) )
end_tm_utc = ( now_tm_utc + datetime.timedelta(minutes= - (days_behind[0] -1)*24*60) )
...
path = "D:\\OneDrive\\Trading\\Data\\{}\\{}".format(cryptopair, time_slice) # use your path
...
for filename in all_files:
    ...
    filename_start_day = int(datepart_of_filename[8:10])
    
    filter_start_year = pd.to_datetime(start_tm_utc).year
    filter_start_month = pd.to_datetime(start_tm_utc).month
    filter_start_day = pd.to_datetime(start_tm_utc).day
    
    filter_end_year = pd.to_datetime(end_tm_utc).year
    filter_end_month = pd.to_datetime(end_tm_utc).month
    filter_end_day = pd.to_datetime(end_tm_utc).day

    #if (filename_start_year >= filter_start_year) & (filename_start_month >= filter_start_month)  & (filename_start_date >= filter_start_day):
    if ( datetime.datetime( filename_start_year, filename_start_month, filename_start_day) >=  datetime.datetime( filter_start_year, filter_start_month, filter_start_day) ) \
        & ( datetime.datetime( filename_start_year, filename_start_month, filename_start_day) <=  datetime.datetime( filter_end_year, filter_end_month, filter_end_day) ):
...
interim_plotdata = alldata_plus_tweets [ ( pd.to_datetime(alldata.timestamp_utc) >= start_tm_utc) & ( pd.to_datetime(alldata.timestamp_utc) <= end_tm_utc) ]

no_of_candlestick_to_view = st.sidebar.slider(' View # of Candlestick(s)', 1 , interim_plotdata.shape[0], (1, interim_plotdata.shape[0])) 



plotdata = interim_plotdata[ -no_of_candlestick_to_view[1] -1 : - no_of_candlestick_to_view[0] ] 
...
conditions  = [ plotdata['is_retweet'] == True, plotdata['reply_to'].isnull(),  ]
choices     = [ 'blue', "green", ]
    
plotdata["tweet_color"] = np.select(conditions, choices, default='black')

plotdata["text_marker"] = plotdata["high"] + d_tweet
plotdata["text_marker_nan"] = plotdata.apply(lambda x : np.nan if np.around(x.pct_change_prev_abs,4)*100 <= 0.8 else x.text_marker, axis =1)


st.write('Replies are  black, re-twets are blue and regular tweets are green') 
...
fig.add_trace( go.Scatter( x = plotdata.timestamp_utc,
                   ...
                   mode='markers',
                   name ='tweet_markers',
                   marker=go.scatter.Marker(size=20, symbol = plotdata.tweet_symbol, color = plotdata.tweet_color) ,
